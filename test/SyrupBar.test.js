...
contract('SyrupBar', ([alice, bob, carol, dev, minter]) => {
  ...
  it('safehalalTransfer', async () => {
    assert.equal(
      (await this.halal.balanceOf(this.syrup.address)).toString(),
      '0'
    );
    await this.halal.mint(this.syrup.address, 1000, { from: minter });
    await this.syrup.safehalalTransfer(bob, 200, { from: minter });
    assert.equal((await this.halal.balanceOf(bob)).toString(), '200');
    assert.equal(
      (await this.halal.balanceOf(this.syrup.address)).toString(),
      '800'
    );
    await this.syrup.safehalalTransfer(bob, 2000, { from: minter });
    assert.equal((await this.halal.balanceOf(bob)).toString(), '1000');
  });
});
